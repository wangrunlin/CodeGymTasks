    ...
    public synchronized String getPartOfString(String string, String threadName) {
        try {
            ...
            if (FIRST_THREAD_NAME.equals(threadName))
                throw new StringForFirstThreadTooShortException(new StringIndexOutOfBoundsException(-1));

            if (SECOND_THREAD_NAME.equals(threadName))
                throw new StringForSecondThreadTooShortException(new StringIndexOutOfBoundsException(-1));

            throw new RuntimeException(new StringIndexOutOfBoundsException(-1));
        }
    }
}
